...
export default async function handler(req, res) {
  try {
    ...
    // 협회/단체 사무실(예: 'OO캠핑장협회'), 캠핑장 안의 부속시설(예: 'OO캠핑장족구장')처럼
    // 실제 야영지가 아닌 곳까지 걸린다. 그래서 실제 야영지 카테고리 표현만 허용하고,
    // 판매/렌탈/용품/협회/부속시설 등은 명시적으로 제외한다.
    const CAMPSITE_PATTERN = /(야영장|캠핑장|글램핑장|글램핑존|오토캠핑장|카라반\s*파크)/;
    const NON_CAMPSITE_PATTERN = /(용품|판매|렌탈|대여|매매|수리|정비|딜러|대리점|전문점|제작|시공|설치|캠핑카|트레일러|협회|연합회|조합|사무국|본부|지회|단체|법인|공단|진흥원|재단|족구장|화장실|샤워장|취사장|정자|야외무대|물놀이장|주차장|관리사무소|안내소|매표소|체육시설|운동장|놀이터|정문|후문|입구|매점)/;
